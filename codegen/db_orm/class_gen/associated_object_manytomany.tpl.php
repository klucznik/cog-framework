...
<?php $manyToManyReferenceTable = $codegen->getTable(strtolower($manyToManyReference->associatedTable)); ?>
	...
	*/
	public function count<?= $manyToManyReference->objectDescriptionPluralUppercase ?>(): int {
		...
		return <?= $manyToManyReference->variableType ?>::countBy<?= $manyToManyReference->oppositeObjectDescription ?>($this-><?= $table->primaryKeyColumnArray[0]->propertyName ?>);
	}

	/**
	 * Checks to see if an association exists with a specific <?= $manyToManyReference->objectDescriptionUppercase ?>

	 * @param <?= $manyToManyReference->variableType ?> $<?= $manyToManyReference->parameterName ?>

	 * @return bool
	 * @throws UndefinedPrimaryKeyException
	 * @throws CogException
	*/
	public function is<?= $manyToManyReference->objectDescriptionUppercase ?>Associated(<?= $manyToManyReference->variableType ?> $<?= $manyToManyReference->parameterName ?>): bool {
		if (<?= $codegen->implodeObjectArray(' || ', 'is_null($this->', ')', 'propertyName', $table->primaryKeyColumnArray) ?>) {
		...
		if (<?= $codegen->implodeObjectArray(' || ', 'is_null($' . $manyToManyReference->parameterName . '->', ')', 'propertyName', $manyToManyReferenceTable->primaryKeyColumnArray) ?>) {
		...
		$intRowCount = <?= $table->className ?>::queryCount(
			QQ::andCondition(
				QQ::equal((new QQNode<?= $table->className ?>)-><?= $table->primaryKeyColumnArray[0]->propertyName ?>, $this-><?= $table->primaryKeyColumnArray[0]->propertyName ?>),
				QQ::equal((new QQNode<?= $table->className ?>)-><?= $manyToManyReference->objectDescription ?>-><?= $manyToManyReference->oppositePropertyName ?>, $<?= $manyToManyReference->parameterName ?>-><?= $manyToManyReferenceTable->primaryKeyColumnArray[0]->propertyName ?>)
			)
		);
	 ...
	 * @throws CogException
	*/
	public function associate<?= $manyToManyReference->objectDescriptionUppercase ?>(<?= $manyToManyReference->variableType ?> $<?= $manyToManyReference->parameterName ?>): void {
		if (<?= $codegen->implodeObjectArray(' || ', 'is_null($this->', ')', 'propertyName', $table->primaryKeyColumnArray) ?>) {
		...
		$database->nonQuery('
			INSERT INTO <?= $escapeIdentifierBegin ?><?= $manyToManyReference->table ?><?= $escapeIdentifierEnd ?> (
				...
				' . $database->sqlVariable($this-><?= $table->primaryKeyColumnArray[0]->propertyName ?>) . ',
				' . $database->sqlVariable($<?= $manyToManyReference->parameterName ?>-><?= $manyToManyReferenceTable->primaryKeyColumnArray[0]->propertyName ?>) . '
			)
		');
	}
	 ...
	 * @throws CogException
	*/
	public function unassociate<?= $manyToManyReference->objectDescriptionUppercase ?>(<?= $manyToManyReference->variableType ?> $<?= $manyToManyReference->parameterName ?>): void {
		if (<?= $codegen->implodeObjectArray(' || ', 'is_null($this->', ')', 'propertyName', $table->primaryKeyColumnArray) ?>) {
		...
		$database->nonQuery('
			...
			WHERE
				<?= $escapeIdentifierBegin ?><?= $manyToManyReference->column ?><?= $escapeIdentifierEnd ?> = ' . $database->sqlVariable($this-><?= $table->primaryKeyColumnArray[0]->propertyName ?>) . ' AND
				<?= $escapeIdentifierBegin ?><?= $manyToManyReference->oppositeColumn ?><?= $escapeIdentifierEnd ?> = ' . $database->sqlVariable($<?= $manyToManyReference->parameterName ?>-><?= $manyToManyReferenceTable->primaryKeyColumnArray[0]->propertyName ?>) . '
		');
	}
	...
	*/
	public function unassociateAll<?= $manyToManyReference->objectDescriptionPluralUppercase ?>(): void {
		...
		$database->nonQuery('
			...
			WHERE
				<?= $escapeIdentifierBegin ?><?= $manyToManyReference->column ?><?= $escapeIdentifierEnd ?> = ' . $database->sqlVariable($this-><?= $table->primaryKeyColumnArray[0]->propertyName ?>) . '
		');
	}
